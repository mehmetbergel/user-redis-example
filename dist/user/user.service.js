...
let UserService = class UserService {
    constructor(userRepository, redisClient) {
        ...
        this.REDIS_KEY_PREFIX = 'user:';
    }
    async createUser(username) {
        const user = this.userRepository.create({ username });
        const savedUser = await this.userRepository.save(user);
        await this.redisClient.set(`${this.REDIS_KEY_PREFIX}:${savedUser.id}`, JSON.stringify(savedUser), 'EX', 30);
        return savedUser;
    }
    async getAllUsers() {
        ...
        await this.redisClient.set(this.REDIS_KEY_PREFIX, JSON.stringify(users), 'EX', 30);
        return users;
    }
};
